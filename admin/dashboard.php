...
<head>
    ...
    <script>
        ...
        (function() {
            ...
            const validTabs = ['reports', 'users', 'settings'];
            const currentTab = validTabs.includes(hash) ? hash : 'reports';

            // Store the current tab so we can use it after DOM loads
            window.initialTab = currentTab;

            // Add inline style to show only the correct tab immediately
            document.write('<style id="preload-tab-style">.tab-content:not(#' + currentTab + ') { display: none !important; } #' + currentTab + ' { display: block !important; }</style>');
        })();
    </script>
</head>
    ...
    <script>
        ...
        function updateSidebarActiveState(tabName) {
            ...
            document.querySelectorAll('.sidebar-link').forEach(link => {
                ...
                link.classList.add('text-text-main', 'dark:text-gray-300', 'hover:bg-background-light', 'dark:hover:bg-slate-700');

                // Update icons
                const icon = link.querySelector('.material-symbols-outlined');
                if (icon) {
                    icon.classList.remove('filled');
                    icon.classList.add('text-text-secondary', 'group-hover:text-primary', 'transition-colors');
                }
            ...
            // Add active state to current tab's sidebar link
            const activeLink = document.querySelector(`.sidebar-link[data-tab="${tabName}"]`);
            if (activeLink) {
                activeLink.classList.add('bg-primary/10', 'text-primary');
                activeLink.classList.remove('text-text-main', 'dark:text-gray-300', 'hover:bg-background-light', 'dark:hover:bg-slate-700');

                // Update icon
                const icon = activeLink.querySelector('.material-symbols-outlined');
                if (icon) {
                    icon.classList.add('filled');
                    icon.classList.remove('text-text-secondary', 'group-hover:text-primary', 'transition-colors');
                }

                // Update text weight
                const text = activeLink.querySelector('span:last-child');
                if (text) {
        ...
        function switchTab(tabName, updateUrl = true) {
            ...
            // Specifically target tab buttons, not sidebar links
        ...
        function initializeTab() {
            // Remove the inline preload style
            const preloadStyle = document.getElementById('preload-tab-style');
            if (preloadStyle) {
                preloadStyle.remove();
            }
            ...
            const validTabs = ['reports', 'users', 'settings'];
            const tabName = validTabs.includes(hash) ? hash : 'reports';
        ...
        // Handle sidebar tab links to prevent page reload when already on dashboard
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.sidebar-link[data-tab]').forEach(link => {
                link.addEventListener('click', function(e) {
                    const tabName = this.getAttribute('data-tab');
                    if (tabName && window.location.pathname.includes('dashboard.php')) {
                        e.preventDefault(); // Prevent page reload
                        switchTab(tabName, true); // Switch tab with URL update
                    }
                });
            });
        });
